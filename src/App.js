...
${themeLabel ? `【今日のテーマ】${themeLabel}` : ""}`;
  ...
  const startPhase2 = async (themeId) => {
    ...
    const openingByTheme = {
      moyo: r
        ? `STEP1で「${v1}」を大切にしていると出ていました。その気持ちと、今の仕事の間でギャップを感じることはありますか？`
        : "最近、仕事でモヤモヤを感じるのはどんな場面ですか？",
      tsuyomi: r
        ? `STEP1で「${s1}」という強みが出ていました。それが一番発揮できた仕事の場面を、一つ具体的に教えてもらえますか？`
        : "これまでの仕事で、一番手応えを感じた場面はどんなときでしたか？",
      taisetu: r
        ? `STEP1で「${v1}」を大切にしていると出ていました。それを実感できた具体的な場面はありますか？`
        : "仕事をしていて「これだけは大切にしたい」と感じることはありますか？",
      tensyoku: r
        ? `STEP1で「${w1}」という方向性が見えていました。今の仕事でそれに近い感覚を得られているときはありますか？`
        : "転職を考えるようになったのは、どんなきっかけがありましたか？",
      career: r
        ? `STEP1で「${w1}」という方向性が出ていました。今の仕事でそれに近い瞬間を感じることはありますか？`
        : "5年後、どんな状態でいたいかイメージはありますか？",
      free: r
        ? `STEP1で「${s1}」という強みと、「${v1}」を大切にしていることが見えてきました。この2つが重なっていると感じる場面は、仕事の中でありますか？`
        : "仕事の中で、最近一番印象に残っていることを教えてもらえますか？",
    };
