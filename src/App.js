...
import {
  ...
  ChevronRight, ChevronLeft, Save, Check, BookOpen, Lightbulb,
  User, BarChart2, ArrowRight, Layers, Target
} from "lucide-react";
...
【絶対に使ってはいけない言い回し】
以下のような不自然な日本語は絶対に使わないでください：
・「〜できてほしいですよね」→NG。「そうなれたらいいですよね」などもNG
...
・「具体的にはどんな場面でしたか？」
・「そのとき、何が一番気になっていましたか？」
・「〇〇というのは、どういうことですか？」（クライアントの言葉をそのまま使う）
...
【文体の厳格なルール】
・自然な日本語の話し言葉で書く（実際の人間のカウンセラーが話すように）
      ...
      {p1step === 4 && (
        <div>
          ...
          <div style={{ background:C.tealL, border:`1px solid ${C.teal}33`, borderRadius:12, padding:"16px 20px", marginBottom:20, fontSize:14, color:C.teal, lineHeight:1.7, display:"flex", gap:12, alignItems:"flex-start" }}>
            <User size={18} color={C.teal} strokeWidth={1.8} style={{ flexShrink:0, marginTop:1 }}/>
            <div>
          ...
          <button
            onClick={()=>{ persist({ basic, careers, skillMap, phase1Done: true }); setPage("dashboard"); }}
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"24px 20px", maxWidth:720, width:"100%", margin:"0 auto", display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{ display:"flex", flexDirection:msg.role==="user"?"row-reverse":"row", gap:10, alignItems:"flex-end" }}>
            {msg.role === "assistant" && (
              <div style={{ width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, color:C.teal }}>
          ...
          {[
            ...
            <div key={card.label} style={{ background:card.bg, border:`1px solid ${card.done?card.color+"44":C.border}`, borderRadius:14, padding:18 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                ...
                {card.done
                  ? <Check size={15} color={card.color} strokeWidth={2.5}/>
                  : <div style={{ width:15, height:15, borderRadius:3, border:`1.5px solid ${C.border}` }}/>
                }
              </div>
